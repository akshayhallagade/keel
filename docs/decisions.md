...
| Todo buckets       | Not stored. Today / This week / Someday are worked out from `dueAt` and the user's `weekStart` every time the screen draws.                                        | Never — a stored bucket is only true on the day it was written, and it went stale by the next morning. Dropped in `20260912083027_drop_todo_bucket`.                   |
...
| Closing an account | Soft delete. `userRepository.closeAccount` tombstones the user and its rows in one transaction; `onDelete: Cascade` stays as a backstop for a genuine hard delete. | Never — a cascade fires on DELETE, never on the UPDATE a soft delete performs, so it can never cover this. Add one `updateMany` per new domain.                        |
| Top 3 todos        | Capped at three, enforced server-side with a 409.                                                                                                                  | The Today screen stops slicing to three. The check is not atomic, so a race could briefly allow a fourth — acceptable for a list the user is looking at.               |
| Todo ordering      | None. Listed newest-first by `createdAt`.                                                                                                                          | Drag-to-reorder is built. Backfilling `position` is one `UPDATE … row_number() OVER (ORDER BY "createdAt")`, so unlike the id format this is genuinely cheap to defer. |
